...
* $\vec{x^{(i)}}$  维度为 $(d_{input}, 1)$；
* $\mathbf{W}$  维度为 $(d, d_{input})$；
* $\vec{z^{(i)}}$ 维度为 $(d, 1)$；
* $\vec{\hat{y}^{(i)}}$ 维度为 $(d, 1)$；
...
* $\frac{\partial l^{(i)}}{\partial \vec{\hat{y}^{(i)}}}$ 维度为 $(d, 1)$；

* $\frac{\partial \vec{\hat{y}^{(i)}}}{\partial \vec{z^{(i)}}}$ 维度为 $(d, d)$；

* $\frac{\partial \vec{z^{(i)}}}{\partial W}=\vec{x^{(i)}}^T$ 维度为 $(1, d_{input})$；

* 最终：$\frac{\partial l^{(i)}}{\partial W}$ 维度为 $(d, d_{input})$；

那么softmax的梯度下降公式为：

$$\begin{equation}w=w - \eta \Big[\frac{1}{N} \sum_{i=1}^N (\vec{\hat{y}^{(i)}} - \vec{y^{(i)}})  \vec{x^{(i)}}^T \Big]\end{equation}$$

之前的二分类较差熵损失的梯度下降公式为：

$$\begin{equation}w= w - \eta \Big[\frac{1}{N} \sum_{i=1}^N (\hat{y}^{(i)} - y^{(i)})  x^{(i)} \Big]\end{equation}$$

均方差损失的梯度下降公式为：

$$\begin{equation}w= w - \eta \Big[\frac{1}{N} \sum_{i=1}^N (\hat{y}^{(i)} - y^{(i)}) \sigma^{\prime}(z^{(i)}) x^{(i)} \Big]\end{equation}$$
