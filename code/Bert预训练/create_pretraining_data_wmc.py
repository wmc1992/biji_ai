...
class Tokenizer:
    ...
    def tokenize(self, line):
        ...
        # 使用jieba分词，并给每个词语非第一个字添加"##"前缀
        segment_list = jieba.lcut(line)
        new_segment_list = []
        for seg in segment_list:
            if len(han_p.findall(seg)) > 0:
                for i in range(len(seg)):
                    if i == 0:
                        new_segment_list.append(seg[i])
                    else:
                        new_segment_list.append("##" + seg[i])
            else:
                new_segment_list += list(seg)

        return new_segment_list

    def convert_tokens_to_ids(self, tokens):
        ids = [self.vocab.get(token, "[UNK]") for token in tokens]
        return ids
...
parser.add_argument("--do_whole_word_mask", default=False, type=bool,
        ...
        for (inst_index, instance) in enumerate(instances):
            ...
            if inst_index < 20:
                ...
                for feature_name in features.keys():
                    ...
                    logging.info("%s: %s" % (feature_name, " ".join([str(x) for x in feature])))
  ...
  """
    ...
    with jsonlines.open(input_file, "r") as f:
        for data in f:
            ...
            if rng.random() < 0.8:  # 80%概率bjnr和cljgnr都要
                tokens = tokenizer.tokenize(data["bjnr"])  # TODO
                all_documents[-1].append(tokens)
                tokens = tokenizer.tokenize(data["cljgnr"])  # TODO
                all_documents[-1].append(tokens)
            else:
                if rng.random() < 0.5:  # 10%概率只要bjnr
                    tokens = tokenizer.tokenize(data["bjnr"])  # TODO
                    all_documents[-1].append(tokens)
                else:  # 10%概率只要cljgnr
                    tokens = tokenizer.tokenize(data["cljgnr"])  # TODO
    ...
    while i < len(document):
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                # 去除给每个词语非第一个字添加的"##"前缀
                tokens = [x if not x.startswith("##") else x[2:] for x in tokens]
                masked_lm_labels = [x if not x.startswith("##") else x[2:] for x in masked_lm_labels]
...
def main():
    ...
    tokenizer = Tokenizer(vocab_path=args.vocab_file, do_lower_case=args.do_lower_case)
